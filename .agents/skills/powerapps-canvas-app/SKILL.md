---
name: powerapps-canvas-app
description: Building Power Apps Canvas Apps — controls, Power Fx formulas, galleries, forms, collections, components, navigation, data binding, and debugging. Use when building, modifying, or explaining any Canvas App, its controls, properties, or Power Fx logic.
license: MIT
---
...
## Confirmed Control Quirks and Gotchas

These are empirically confirmed bugs and non-obvious behaviours. Check this section before assuming a pattern works.

---

### Transparent tap/click overlay — use Classic Button (not modern Button)

**Classic Button is the correct choice for transparent overlay controls** (e.g. a tap overlay covering a gallery card).

The modern Button's `Appearance: Transparent` has inconsistent behaviour and less state control. Classic Button gives full, predictable control over every visual state.

**Required properties for a fully transparent Classic Button overlay:**

```powerfx
btnOverlay.Fill               = RGBA(0, 0, 0, 0)         // transparent at rest
btnOverlay.HoverFill          = RGBA(255, 255, 255, 0.08) // subtle hover tint
btnOverlay.PressedFill        = RGBA(255, 255, 255, 0.15) // press feedback
btnOverlay.BorderColor        = RGBA(0, 0, 0, 0)         // no border
btnOverlay.BorderThickness    = 0
btnOverlay.Color              = RGBA(0, 0, 0, 0)         // hide button text
btnOverlay.DisabledFill       = RGBA(0, 0, 0, 0)
btnOverlay.FocusedBorderColor = RGBA(0, 0, 0, 0)
btnOverlay.Text               = ""
```

**Why it appeared solid black in a dark theme:**
The default Classic Button fill inherits the theme's button colour. Setting only `Fill` to transparent is not enough — `HoverFill` and `PressedFill` also need to be set, or Power Apps will fall back to the theme default (which in a dark theme renders as a dark/black surface).

**Pattern — full overlay button on a gallery card:**

```powerfx
// Classic Button sized to cover the full card, placed last in Children so it sits on top (z-order)

btnCardOverlay.Fill               = RGBA(0, 0, 0, 0)
btnCardOverlay.HoverFill          = RGBA(255, 255, 255, 0.06)
btnCardOverlay.PressedFill        = RGBA(255, 255, 255, 0.12)
btnCardOverlay.BorderColor        = RGBA(0, 0, 0, 0)
btnCardOverlay.BorderThickness    = 0
btnCardOverlay.Color              = RGBA(0, 0, 0, 0)
btnCardOverlay.DisabledFill       = RGBA(0, 0, 0, 0)
btnCardOverlay.FocusedBorderColor = RGBA(0, 0, 0, 0)
btnCardOverlay.Text               = ""
btnCardOverlay.Width              = Parent.Width - 16
btnCardOverlay.Height             = Parent.TemplateHeight - 8
btnCardOverlay.X                  = 8
btnCardOverlay.Y                  = 4
btnCardOverlay.OnSelect           = Set(varSelectedTask, ThisItem); Navigate(DetailScreen, ScreenTransition.Fade)
btnCardOverlay.AccessibleLabel    = "Open: " & ThisItem.Title
```

---

### AutoLayout child rendering order is NOT reversed

Children in a `GroupContainer` AutoLayout render **in the same order as their YAML definition** — first child = leftmost (horizontal) or topmost (vertical).

There is no reversal quirk. Any "reverse order" observation was incorrect.

---

### RGBA() inline in screen `.pa.yaml` files causes `YamlInvalidSyntax`

Non-zero RGBA values written inline in screen files (not `App.pa.yaml`) cause YAML parse errors.

```yaml
# BAD — causes YamlInvalidSyntax in screen files
Fill: =RGBA(14, 16, 36, 1)

# GOOD — define a named constant in App.pa.yaml Formulas block
# In App.pa.yaml Formulas:
#   ColorHeaderBg = RGBA(14, 16, 36, 1)
# Then reference it in screen files:
Fill: =ColorHeaderBg
```

`RGBA(0,0,0,0)` (fully transparent) works inline in screen files. Only non-zero alpha values are affected.

---

### Colon-space inside string literals causes YAML parse error

If a Power Fx string contains `: ` (colon followed by a space), the YAML plain scalar parser interprets it as a key-value separator.

```yaml
# BAD — YAML parse error
AccessibleLabel: ="Open task: " & ThisItem.Title

# GOOD — use a block scalar (|- strips the trailing newline)
AccessibleLabel: |-
...
### ModernText and Badge property names differ from classic controls

| Control | Wrong | Correct |
|---|---|---|
| `ModernText` | `FontSize` | `Size` |
| `ModernText` | `FontColor` | `Color` |
| `Badge` | (auto-stretches) | Add `FillPortions: =0` to prevent stretching in AutoLayout |

---

### GroupContainer default is NOT transparent

Always set these on every non-background container:
...
### AccessibleLabel and TabIndex — accessibility checker rules

- `AccessibleLabel: =""` (empty string) **still fails** the checker — use a real descriptive string
- `TabIndex: =-1` **still fails** the checker — use `=0` even for decorative shapes (Rectangle, background fills)
